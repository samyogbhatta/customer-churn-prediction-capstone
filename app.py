...
# Initialize base global filtered dataframe state
...
if app_mode != "Batch Prediction & Risk Explorer":
    ...
    X_filtered_processed = explainer.get_preprocessed_df(filtered_df.drop(columns=["churn"], errors="ignore"))
...
# =====================================================================
# NEW: HORIZONTAL DASHBOARD CONTROLS PANEL (MAIN PAGE PLACEMENT)
# =====================================================================
if app_mode in ["Executive Overview", "Database Explorer"]:
    st.markdown("### Filter Segment Options")
    
    # 3 horizontal columns inside the main page flow
    ctrl_col1, ctrl_col2, ctrl_col3 = st.columns(3)
    
    with ctrl_col1:
        genders = ["All"] + sorted(list(df_raw["gender"].dropna().unique()))
        gender_filter = st.selectbox(" Gender Filter", genders, index=0)

    with ctrl_col2:
        provinces = ["All"] + sorted(list(df_raw["province"].dropna().unique()))
        province_filter = st.selectbox("Province Filter", provinces, index=0)

    with ctrl_col3:
        sim_types = ["All"] + sorted(list(df_raw["sim_type"].dropna().unique()))
        sim_filter = st.selectbox("SIM Type Filter", sim_types, index=0)

    # Filter Dataset downstream reactively based on horizontal filters
...
if app_mode == "Executive Overview":
    ...
    if len(filtered_df) == 0:
        st.warning("No records found matching selected segment variations. Please reset filters above.")
    else:
        ...
        def compute_global_importance(_explainer, df_sample):       
            X_sample_processed = _explainer.get_preprocessed_df(df_sample.drop(columns=["churn"], errors="ignore"))
        ...
        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown("###  Global Feature Distributions (Beeswarm / Summary)")
        ...
        st.markdown("###  Feature Dependence (e.g., Age vs. Churn Impact)")
        fig_dependence = plot_dependence("age", shap_vals, X_sample_processed) 
...
# ----------------- MODE 2: DATABASE EXPLORER -----------------
    ...
    if len(filtered_df) == 0:
        st.warning("No customers match the active segment filters. Reset filters above to explore.")
    else:
        ...
        with st.spinner("Analyzing prediction explainability..."):
            raw_input_df = customer_row.drop(columns=["churn", "customer_id"], errors="ignore")
        ...
        with col_pred2:
            ...
            # FIXED: Crucial bug update to prevent matplotlib 'Figure object savefig' errors
...
# ----------------- MODE: BATCH PREDICTION & RISK EXPLORER -----------------
    ...
    st.markdown("###  Step 1: Download & Prepare Your Data")
        ...
        try:
            ...
            with tab_drivers:
                ...
                with st.spinner("Analyzing global segment driver importance..."):
                    shap_vals, importance_up_df, X_up_processed, sample_len = compute_uploaded_global_shap(uploaded_df, explainer)
                ...
                st.markdown("<hr>", unsafe_allow_html=True)
                st.markdown("####  Segment Feature Distributions (Beeswarm)")
                fig_up_summary = plot_summary(shap_vals, X_up_processed)
                st.plotly_chart(fig_up_summary, use_container_width=True)

                st.markdown("####  Segment Feature Importance (Mean SHAP)")
                fig_up_bar = plot_mean_bar(shap_vals, X_up_processed)
                st.plotly_chart(fig_up_bar, use_container_width=True)

                st.markdown("####  Segment Prediction Path (Waterfall Plot)")
                fig_up_waterfall = plot_waterfall(explanation["shap_values_obj"], max_display=8)
                # FIXED: Changed to use native plotly method mapping
                st.plotly_chart(fig_up_waterfall, use_container_width=True)
...
# ----------------- MODE 3: WHAT-IF SIMULATOR -----------------
    ...
    with st.spinner("Simulating..."):
        explanation = explainer.explain_instance(sim_row.drop(columns=["customer_id"], errors="ignore"))
        prob = explanation["probability"]
        contributions = explanation["contributions"]
